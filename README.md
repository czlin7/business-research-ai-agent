...
* Local inference with [Qwen3.5-4B](https://huggingface.co/Qwen/Qwen3.5-4B), using the `qwen3.5:4b-q4_K_M` Ollama model
...
The prototype uses **Qwen3.5-4B** with the **Q4_K_M** quantization. The model
is downloaded and managed by [Ollama](https://ollama.com/), which runs the model
on the local machine and exposes a local API at
`http://127.0.0.1:11434`. The Python application sends prompts to that local
endpoint; model inference is not sent to a hosted API.
